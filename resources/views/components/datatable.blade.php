            ...
            <table id="{{$id}}" class="table color-bordered-table table-striped full-color-table full-info-table hover-table" data-display-length='-1' data-order="[]">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>@if(isset($item->{$att['verified']['att']})){{ $item->{$att['verified']['att']} }}
                        ...
                        <td>@if(!is_null($item->{$att['foreignUrl']['rel']}))<a href="{{ url($att['foreignUrl']['baseUrl'] . '/' . $item->{$att['foreignUrl']['urlAtt']}) }}">{{
                                $item->{$att['foreignUrl']['rel']}->{$att['foreignUrl']['att']} }}@endif</a>
                        </td>
                        @elseif(array_key_exists('foreignForeign', $att))
                        <td>{{ $item->{$att['foreignForeign']['rel1']}->{$att['foreignForeign']['rel2']}->{$att['foreignForeign']['att']} }}
                        </td>
                        ...
                        <td>
                            <button type="button" style="padding:.1rem" class="btn btn-secondary" data-container="body" data-toggle="popover" data-placement="bottom" data-content="{{$item->{$att['comment']['att']} }}"
                                data-original-title="{{ $att['comment']['title'] ?? 'Comment:'}}"> <i class="far fa-list-alt"></i>
                            </button>
                        </td>
