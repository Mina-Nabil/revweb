...
class OfferRequest extends Model
{
    ...
    {
        $repliedToOfferRequestIDs = DB::table('offers_requests')
            ->selectRaw('DISTINCT offers_requests.id')
            ->join('offers', 'offers.OFFR_OFRQ_ID', '=', 'offers_requests.id')
            ->where('OFFR_SHRM_ID', $showroomID)
            ->get()->pluck('id');
        ...
        $query = self::selectRaw("DISTINCT offers_requests.id, offers_requests.*")->join("offers_requests_colors as offerDetails", "offers_requests.id", "=", "OFRC_OFRQ_ID")
            ...
            })->whereIn("OFRQ_STTS", [OfferRequest::NEW_KEY, OfferRequest::REPLIED_KEY])
            ->whereNotIn('offers_requests.id', $repliedToOfferRequestIDs)
            ->whereDate("created_at", ">", (new Carbon("now"))->subWeekdays(14));
